...
{
    public class LtiUtility : IDisposable
    {
        private ConsumerContext db = new ConsumerContext();

        public LtiRequest CreateLtiRequest(Assignment assignment)
        {
            ...
            {
                Action = uri.ToString(),
                Fields = HttpUtility.ParseQueryString(parameters.ToQueryStringFormat()),
                Signature = signatureProvider.ComputeSignature(signatureBase, assignment.Secret, string.Empty),
        ...
        private void AddOptionalParameters(Assignment assignment, OAuthParameters parameters)
        {
            ...
            parameters.AdditionalParameters.Add("user_id", WebSecurity.CurrentUserName);
        ...
        private void AddLti11Parameters(Assignment assignment, OAuthParameters parameters)
        {
            ...
            parameters.AdditionalParameters.Add("lis_outcome_service_url",
                urlHelper.Action("outcomes", "api", null, HttpContext.Current.Request.Url.Scheme));
            parameters.AdditionalParameters.Add("lis_result_sourcedid",
                string.Format("{0}-{1}", assignment.AssignmentId, user.UserId));
        }
